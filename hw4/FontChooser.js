class FontChooser extends React.Component {
  ...
  handleTextDoubleClick (event) {
    ...
    if (this.state.timeoutID != []) {
      const tIDs = this.state.timeoutID
  ...
  handleCheckBox (event) {
    ...
    this.setState((state) => ({ fontWeight: state.fontWeight === 'normal' ? 'bold' : 'normal' })
    )
  }
  ...
  render () {
    return (
      <div>
        <span id='textSpan' onClick={this.handleTextClick} onDoubleClick={this.handleTextDoubleClick} style={{ fontStyle: this.state.fontStyle, color: this.state.fontColor, fontSize: this.state.fontSize, fontWeight: this.state.fontWeight }}>
          {this.props.text}
        </span>
        <br />
        <label hidden={this.state.hideForm}>
          <input type='checkbox' id='boldCheckbox' checked={this.state.fontWeight == 'bold'} onChange={this.handleCheckBox} />
					Bold
        </label>
